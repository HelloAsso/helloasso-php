...
class HelloAssoApiV5CommonModelsStatisticsOrderPayment implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets is_refundable
     *
     * @param bool|null $is_refundable Indicate if the payment is refundable or not
     *
     * @return self
     */
    public function setIsRefundable($is_refundable)
    {
        if (is_null($is_refundable)) {
            array_push($this->openAPINullablesSetToNull, 'is_refundable');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('is_refundable', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
